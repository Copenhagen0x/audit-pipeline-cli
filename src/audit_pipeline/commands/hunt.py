...
# Reviewer HIGH (P7 R5c): consolidated with assembly.py + health.py via
# the shared `notifier.validate_webhook_url` — was previously a private
# `_webhook_url_safe` + duplicate regex pair here. Single source of
# truth means any future hardening (e.g. removing webhook.site,
# blocking IPv4-mapped IPv6, redirect-following protection) covers
# every webhook dispatch in one place.
def _webhook_url_safe(url: str) -> tuple[bool, str]:
    """Thin wrapper for backwards-compatible callers — delegates to the
    canonical `notifier.validate_webhook_url`."""
...
def _post_webhook(
    ...
    from audit_pipeline.notifier import validate_webhook_url
    allowed, reason = validate_webhook_url(url)
    if not allowed:
    ...
    payload = {"text": "\n".join(msg_lines)}
    # Threat-modeler HIGH (P7 R5c): `allow_redirects=False` keeps
    # validate_webhook_url's decision authoritative — without it, an
    # allow-listed webhook could 301 to http://169.254.169.254/.
    requests.post(url, json=payload, timeout=15, allow_redirects=False)
