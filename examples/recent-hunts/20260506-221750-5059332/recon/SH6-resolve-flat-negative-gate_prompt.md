...
ID:           SH6-resolve-flat-negative-gate
Claim:        `resolve_flat_negative` (engine `src/percolator.rs:4770-4785`) is reached only via `touch_account_live_local` step 7 (`src/percolator.rs:4822-4848`) when `eff_pos == 0 && pnl < 0`. Equivalently: there is no other call site that reaches `absorb_protocol_loss(|pnl|)` → `use_insurance_buffer` from a permissionless wrapper instruction without first touching the losing account.
