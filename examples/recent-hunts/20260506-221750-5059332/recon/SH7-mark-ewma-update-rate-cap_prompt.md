...
- /tmp/jelleo-snap-aeyakovenko-percolator-k1w8zquv/aeyakovenko-percolator-5059332/src/ (all .rs files)
...
ID:           SH7-mark-ewma-update-rate-cap
Claim:        The wrapper's `mark_ewma_e6` update at `src/percolator.rs:6746-6776` clamps per-slot mark divergence to ≤ 49 bps/slot under §1.4 envelope-ceiling RiskParams. No sequence of legitimate `TradeNoCpi` / `TradeCpi` calls can produce a `mark_ewma` value whose divergence from `last_effective_price_e6` produces a funding rate magnitude exceeding the spec'd cap when integrated over any single accrue step.
