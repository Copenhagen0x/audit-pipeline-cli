...
- /tmp/jelleo-snap-aeyakovenko-percolator-k1w8zquv/aeyakovenko-percolator-5059332/src/ (for the engine state struct)
...
ID:           SH12-insurance-drain-via-resolve-flat-negative
Claim:        The insurance buffer cannot be drained by a sequence of permissionless instructions that does NOT include partial liquidation of every open exposed position whose lazy MTM has crossed the maintenance margin threshold. Equivalently: every `use_insurance_buffer` call reachable from a permissionless wrapper handler is preceded in the same call tree by a `partial_liq` evaluation against the candidate account set.
