...
ID:           SH11-self-matched-pair-cannot-walk-K
Claim:        A self-matched pair (attacker keypairs A long / B short opened via `TradeNoCpi` at baseline price) cannot subsequently walk K-state such that one side's lazy MTM exceeds its capital while the other side accumulates a haircut-claimable matching positive PnL, regardless of which permissionless instruction sequence the attacker uses (decoy WithdrawCollateral, CatchupAccrue, KeeperCrank, TradeCpi with attacker matcher).
