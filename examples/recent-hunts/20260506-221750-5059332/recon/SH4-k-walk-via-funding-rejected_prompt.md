...
ID:           SH4-k-walk-via-funding-rejected
Claim:        A multi-day warp under static Pyth oracle but non-zero funding rate (driven by `mark_ewma` divergence from attacker-controlled CPI matcher at exec_price = oracle × 1.005) cannot accumulate K-state walk via `compute_current_funding_rate_e9` × elapsed_slots without an account- touching gate firing on every materialized exposed account before risk decisions.
