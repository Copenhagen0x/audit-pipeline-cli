...
ID:           SH5-keeper-crank-touching-completeness
Claim:        `KeeperCrank` (`src/percolator.rs:5434-5673`) commits partial-catchup WITH per-candidate `touch_account_live_local` (engine `src/percolator.rs:4822-4848`) on every account in its candidate set before any `accrue_market_to` further advances the market clock. This invariant guarantees that lazy MTM realized on the candidate cannot bypass `partial_liq` enforcement.
