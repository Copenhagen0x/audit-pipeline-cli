...
Claim:        Every code path that mutates `insurance_fund.balance` is paired with an equal-magnitude mutation of `vault` in the same direction (credit insurance => credit vault; debit insurance => debit vault).
