...
ID:           W2-resolve-flat-negative-via-touch-public
Claim:        resolve_flat_negative_with_context at percolator.rs:7123 calls absorb_protocol_loss(loss) when an account has position_basis_q==0 and pnl<0. This is reachable from touch_account_live_local (line 7191) which is in turn reachable from keeper_crank_not_atomic (the engine helper called by KeeperCrank wrapper instruction). Verify whether an attacker can construct (account_idx, pnl, position_basis) state where this path fires WITHOUT a bankruptcy that legitimately required insurance absorption. Specifically: does any sequence of public calls let an attacker leave an account with position_basis_q=0 AND pnl<0 in such a way that resolve_flat_negative drains insurance disproportionately to the actual protocol loss?
