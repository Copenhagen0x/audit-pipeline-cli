...
ID:           W4-permissionless-progress-bypass-bankruptcy-lock
Claim:        permissionless_progress_not_atomic at percolator.rs:8754 dispatches to keeper_crank_with_request_not_atomic, which can trigger touch_account_live_local on candidates. Verify the bankruptcy_hmax_lock mechanism actually prevents subsequent insurance-decreasing operations when set, or whether an attacker can race progress into the same crank that triggered the lock and drain insurance further before the lock takes effect on subsequent calls.
