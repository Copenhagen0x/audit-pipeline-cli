...
ID:           W6-conservation-postcondition-coverage
Claim:        assert_public_postconditions is called at the end of every public mutating function. Verify whether this postcondition includes a vault >= c_tot + insurance_fund.balance check (residual conservation), AND whether that check is strict enough to catch the F7 helper-level violation. If absorb_protocol_loss runs and insurance shrinks without an equivalent vault or c_tot change, the postcondition should fail and roll back the entire instruction. If the postcondition allows insurance to shrink while vault stays constant, the helper-level violation is reachable from the public surface.
