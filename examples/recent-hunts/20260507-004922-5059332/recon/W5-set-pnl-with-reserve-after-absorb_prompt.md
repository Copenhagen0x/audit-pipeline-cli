...
ID:           W5-set-pnl-with-reserve-after-absorb
Claim:        resolve_flat_negative_with_context at line 7148 calls set_pnl_with_reserve(idx, 0, NoPositiveIncreaseAllowed, None) AFTER absorb_protocol_loss. This zeros the negative PnL. Verify the order: absorb (debits insurance) → set PnL to 0 (resets account). If the sequence allows the SAME account to be re-touched and re-absorb in a subsequent call (e.g., if pnl somehow becomes negative again without a real loss event), insurance is drained again. Check that account state after resolve_flat_negative cannot re-enter the bankruptcy path without a new real loss.
