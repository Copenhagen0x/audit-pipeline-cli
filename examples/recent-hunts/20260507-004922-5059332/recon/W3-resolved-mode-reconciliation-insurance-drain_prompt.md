...
ID:           W3-resolved-mode-reconciliation-insurance-drain
Claim:        In Resolved-mode reconciliation, force_close_resolved_with_fee_not_atomic and reconcile_resolved_not_atomic eventually call settle_losses + resolve_flat_negative which calls absorb_protocol_loss. After ResolveMarket flips market_mode to Resolved, anyone can call these reconciliation paths permissionlessly. Verify whether any sequence of pre-resolve account state, ResolveMarket call, and post-resolve reconciliation call can drain insurance beyond the legitimate pre-resolve bankruptcy footprint. Specifically: if pre_residual was positive and resolved-mode drains it, the insurance should be debited by exactly the residual; any excess drain is a bug.
