...
ID:           SH9-stuck-target-accrual-rejection
Claim:        `reject_stuck_target_accrual` correctly rejects every accrue path that would advance market state past a configured staleness window under stuck-oracle conditions, AND this rejection cannot be bypassed by the attacker setting `funding_rate_e9` to a value that would make `compute_current_funding_rate_e9` skip the staleness branch.
