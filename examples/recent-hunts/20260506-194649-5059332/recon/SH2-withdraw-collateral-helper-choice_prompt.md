...
- BPF instructions of note: WithdrawCollateral handler at src/percolator.rs:5756
...
- **`WithdrawCollateral handler at src/percolator.rs:5756
...
- /tmp/jelleo-snap-aeyakovenko-percolator-li_9z8kf/aeyakovenko-percolator-5059332/src/ (focus on functions that mutate multiple fields)
...
ID:           SH2-withdraw-collateral-helper-choice
Claim:        `WithdrawCollateral` invokes `ensure_market_accrued_to_now_for_account_limited_op` (the strict helper) and not `ensure_market_accrued_to_now_with_policy`, because WithdrawCollateral admits lazy MTM accumulation on user accounts that are not the signer's account (e.g., a decoy walk).
