...
Claim:        An account's `position_q` and `claimable_pnl` can only be mutated when the account's bound authority signs (or via permissionless instructions that explicitly preserve net-positive equity).
