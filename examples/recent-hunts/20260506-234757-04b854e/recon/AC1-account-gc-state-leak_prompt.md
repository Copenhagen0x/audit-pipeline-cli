...
Claim:        free_slot / reclaim_empty_account zeroes every byte of the account slot, so a subsequent materialize_at on the same index cannot inherit shadow state (PnL, capital, position basis) from the prior owner.
