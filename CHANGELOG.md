...
### A crashed audit no longer reports success (2026-07-25)
- **`hunt` now exits non-zero when Layer 1 dies.** Previously, when recon produced no summary — timeout (rc=124), crash, missing binary (rc=127), or a kill mid-run — hunt printed a red error, finished the cycle with 0 dispatched, and then did a bare `return`, which click reports as **exit 0**. Every unattended runner therefore read a DEAD audit as a CLEAN one. The message now names the cause ("recon timed out" / "recon binary not found" / "recon did not run") and the process exits 1.
- **Why this branch is unambiguously a failure and not a no-op:** `recon` writes `recon_summary.json` unconditionally — even when every hypothesis is filtered out of scope and even when every verdict is FALSE (verified empirically by running recon with an empty library: summary written, exit 0). The file is missing ONLY when recon itself died. Legitimate quiet runs still exit 0, so this cannot turn a green nightly red for a docs-only commit.
- **What it prevents (latent, not a live incident):** `watch` gates its WS6 diff-scope baseline on `audit_ok = returncode == 0`, so a crashed run WOULD advance `last_scoped_sha` past a commit that was never audited — permanently excluding that commit from every future scan. It cannot bite in the currently deployed config (`jelleo-watch.service` passes no `--diff-scope`; `watch_on_update.sh` is the neutered shim), but the hole opens the moment watch is re-armed with a real hunt + `--diff-scope`.
- **Wrapper caveat:** a shell wrapper that pipes hunt through `tee` discards this exit code (bash reports the last pipeline stage). `deploy/restart_watch_with_hunt.sh` does exactly that — any wrapper must use `set -o pipefail` or `${PIPESTATUS[0]}` or the fix is silently undone.
- **NOT covered (tracked follow-up):** a total API outage. recon catches per-hypothesis exceptions as data, still writes a summary (all verdicts `status=ERROR`) and exits 0 — so an audit where nothing was successfully checked still reports clean. Closing that needs a separate check on the summary (`n_ok == 0` while `n_hypotheses > 0`).
- No caller retries on the new failure signal (verified: `converge` breaks, `watch` advances `last_seen_sha` unconditionally so the same commit is never re-fired, and no systemd unit invokes hunt) — no crashloop risk. Tests: `tests/test_hunt_fails_on_dead_audit.py` (6), driving the real command; every guard mutation-tested. Also corrected two now-false comments (`converge.py`) that documented the old exit-0 behaviour as truth — that comment drift is how the original bug survived.

### WS4 proof-carrying by default — proof now gates DISCLOSURE (2026-07-17)
- **The engine no longer discloses a Critical/High finding it hasn't proven.** Before this, a finding was confirmed, cryptographically signed, and published to the public archive / a GitHub issue / a customer email **without ever checking the formal-proof outcome** — a Kani-disproved, cannot-model, or never-run finding went out identically to a proven one.
- **"Proof-carrying" = Kani formally proved the invariant OR the exploit was demonstrated at runtime** (LiteSVM `fired`, or the generic runtime-fuzz adapter's `crash_found` for non-Solana targets). EITHER suffices — many Solana bugs cannot be Kani-modeled but ARE L4-exploit-confirmed, so requiring Kani alone would bury real demonstrated exploits (operator decision 2026-07-17). L2 `poc_fired` alone is NOT proof. The predicate is **fail-closed**: a missing / unreadable / counterexample / cannot-verify / returncode-only proof block never counts.
- **Signing is untouched; the gate holds PUBLISH.** The proof outcome (kani + litesvm) is now persisted per-finding in `details_json`, so it is committed tamper-evidently into the signed Merkle leaf. A Critical/High that isn't proof-carrying is therefore **signed but held from disclosure** until it earns a proof or an exploit.
- **The gate holds at EVERY door out of the firm**, via one shared predicate (`gates.post_cycle.unproven_block_reason`): auto-publish (`check_post_cycle`, default `require_proof_for_high=True`), `issue file` (refuses, fail-closed), `issue auto-file-confirmed` (skips), and the customer-alert email `notify critical` (refuses). It gates only the findings that will **actually** publish (the L4-witnessed `confirmed` set, not every `poc_fired` row — otherwise an L4-refuted finding would hold the whole cycle). Each override (`--allow-unproven-publish`, on `hunt` / `issue file` / `notify critical`) emits a structured event (`published_/disclosed_/emailed_unproven_override`) so it is never a silent bypass.
- **"Runtime exploit" requires a concrete abort/failure witness**, consistent across adapters: Solana `litesvm.fired` (test FAILED + engine-authored witness) or the generic runtime-fuzz adapter's `crash_found` with `ran_clean=False` (the inverted-assertion harness aborted). A clean end-to-end PASS (`crash_found` + `ran_clean=True`) is the weaker, harness-correctness-dependent case and does NOT count on its own — it is held for a Kani proof or an abort witness.
- **Fails closed on gate error** — an exception in the gate now writes the block sentinel and aborts auto-publish, rather than falling through to publish. **The override is attested** — `--allow-unproven-publish` (on both `hunt` and `issue file`) logs a structured `published_unproven_override` event naming the finding ids, so it is never a silent bypass.
- **KNOWN LIMITS (tracked, not fixed here — WS4 is landed on `feat/ws4-proof-gating` as an increment, NOT yet merged to main):** (1) a mid-cycle-crash `--resume-cycle` can drop or forge a proof via the wholesale `details_json` rewrite + the unsigned pre-resume summary + the moved `.publish-blocked` sentinel — fails closed (over-holds), task-chipped; (2) `kani.proved`/`litesvm.fired`/`crash_found` derive from text-matching an LLM-authored harness, a spoofing surface WS4 elevates, task-chipped; (3) the confirm step (`hunt.py` `confirmed` build) still credits only Solana `l4.fired`, so a non-Solana `crash_found` finding is dropped BEFORE the auto-publish gate (the `crash_found` predicate branch is exercised via the disclosure exits, which read findings directly) — safe direction, follow-up; (4) `disclose.py` (renders docs from an operator-authored YAML, no DB) is an ungated human-driven door — lower priority. **Intentional non-gates:** the `bundle merged` walk (confirmed→disclosed→fixed) is NOT proof-gated because it fires only when an upstream maintainer merged the fix PR — human acceptance of the fix is stronger validation than an automated proof, so gating it would wrongly hold a validated finding. Tests: `tests/test_ws4_proof_gating.py` (28) incl. a real-FindingsDB round-trip and all four disclosure/email exits; every guard mutation-tested.
